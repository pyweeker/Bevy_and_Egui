...
fn main() {
    App::new()
        ...
        //.add_startup_system(ui_example.system())
...
struct UiState {
    ...
    //vec_entities_colors: Vec<Color::hsla>,
    //vec_entities_colors: Vec<bevy::render::color::Color::Hsla>,
...
impl Default for UiState {
    fn default() -> Self {
        ...
        UiState { value: 3.5, rotval: 0.0, vertimov: 0.0, painting: Painting::default(), inverted: true, name: "the_name".to_string(), age: 18, current_target_idx: None, current_png_String: "".to_string(), vec_entities: Vec::<Entity>::new(), vec_texturenames: Vec::<&str>::new(), vec_entities_colors: Vec::<bevy::render::color::Color>::new()}
    }
}
...
//fn make_colored_rect(mut commands: Commands, the_color: Color::hsla, the_transform: Transform::from_translation) {
...
fn keyboard_system(
    ...
    //mut commands: Commands,    value moved here, in previous iteration of loop  ===XXX>  if idx_of_e.idx == current_usize {make_colored_rect_with_transform(commands, tf.clone());}
    ...
    if keyboard_input.just_pressed(KeyCode::Right) {
        ...
        if ui_state.current_target_idx.is_none() {
            ...
            for (idx_of_e, texturename, tf) in queryIdx_Nametextur_and_Transform.iter() {  
                ...
                //    );
                // commands value moved here, in previous iteration of loop

                //if idx_of_e.idx == current_usize {make_colored_rect_with_transform(commands, tf.clone());}
                ...
                if idx_of_e.idx == current_usize {
                    ...
                    //colored_rect.insert(Cleaner_job);
...
fn clean_colored_rect_system(
    ...
    mut query_cleanable: Query<(Entity, &Cleaner_job)>,
        ...
        //let current_usize: usize = ui_state.current_target_idx.unwrap();  // panicked at 'called `Option::unwrap()` on a `None` value'
        //if let current_usize: usize = ui_state.current_target_idx.unwrap();

        //if let current_usize: Option::usize = ui_state.current_target_idx.unwrap();

        //for (e, cj) in query_cleanable.iter() {

        //    if let Some(current_usize.unwrap()) = cj.index_of_owner { commands.entity(e).despawn(); }
        ...
        match current_usize {
            ...
            //current_usize.unwrap().is_none() => println!("The maximum is configured to be "),
            None => println!("current_usize is None"),
