...
fn main() {
    App::new()
        ...
        .insert_resource(Scoreboard { score: None })
...
impl Default for UiState {
    fn default() -> Self {
        ...
        UiState { value: 3.5, rotval: 0.0, vertimov: 0.0, painting: Painting::default(), inverted: true, name: "the_name".to_string(), age: 18, current_target_idx: None, current_target_idx_indicator: "No Target Yet".to_string()}
    }
}
...
fn scoreboard_system(scoreboard: Res<Scoreboard>, mut query: Query<&mut Text>) {
    ...
    //text.sections[1].value = format!("{}", scoreboard.score.unwrap());

    //if let Some(s) = scoreboard.score.unwrap() {
    if let Some(s) = scoreboard.score {
        println!("{}", &s);
        text.sections[1].value = format!("{}", scoreboard.score.unwrap());
    }

    //else { print_type_of(&scoreboard.score); }
}